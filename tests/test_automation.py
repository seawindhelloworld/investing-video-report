...
class AutomationTests(unittest.TestCase):
    ...
    def test_prompt_requires_full_workflow_without_git_or_subagents(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("不启动字幕勘误、风险词扫描、额外模型调用或派生字幕", prompt)
            self.assertIn("最终报告不展示 ASR 风险清单", prompt)
            self.assertIn("广告、推广、订阅和销售话术不进入观点、研究或正文", prompt)
            self.assertIn("编辑标题", prompt)
            self.assertIn("不另起模型调用或规划文件", prompt)
            self.assertIn("财经杂志式表达", prompt)
            self.assertIn("强封面、主题导读、行情/KPI卡、人物观点卡、机制图", prompt)
            self.assertIn("不强制时间戳、claim ID、审计映射", prompt)
            self.assertIn("完整结构化数据继续由 report-data.json 与 citations.json 承担", prompt)
