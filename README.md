...
| 模式 | 输入 | 输出逻辑 |
| --- | --- | --- |
| 视频报告 | 一个通过结构、哈希与上游审计校验的标准字幕包 | 用强封面、主题导读、人物观点卡与机制图整理作者内容，再做外部证据研判与 Agent 综合判断 |
...
页面中的模型和推理强度都是下拉框。Codex 模式读取本机 Codex 模型清单，并根据所选模型联动可用的推理强度；视频报告把这个选择视为上限，原意分析、外部研究和起草最高使用 `high`，综合判断最高使用 `xhigh`，原意审查最高使用 `medium`，用户选择更低强度时保持更低值。选择 GPT-5.6 Sol 时还可启用 Fast 模式，服务层不可用会自动回退到标准模式。OpenCode 模式固定显示本机已安装的 DeepSeek V4 Flash 与 DeepSeek V4 Pro，并把各模型支持的推理选项作为 `variant` 传入。两款 DeepSeek V4 模型目前均只接收文字输入，含图片素材时应选择标明支持图片的 Codex 模型。

任务运行期间会显示阶段完成比例、当前动作、阶段耗时、最近事件和服务心跳。视频模式会为 `analyze`、`research`、`judgment`、`draft`、`fidelity_review` 分别启动顺序、隔离的模型会话，并在每次调用前立刻更新当前阶段；只有 `research` 允许联网。页面日志来自 Codex JSON 事件的精简视图，完整原始 JSONL 单独保留并可按需打开，避免大段 diff 或模型输出让页面看似卡死。模型阶段结束后，外层程序确定性执行结构化合并与 `render`，再用本机真实浏览器检查桌面和移动视口并自动完成 `html_validate`、`complete`；恢复任务只从首个未完成阶段继续，已经完成的任务默认直接复用且页面会明确显示“未调用模型”。如需比较改造前后的报告，在提交同一视频字幕包前勾选“重新生成并保留旧版”：程序会先保存旧版完整快照，再从原意分析开始运行，完成后同时提供“打开完整报告”和“打开旧版对比”。同一次服务运行中提交的报告会进入页面任务列表，并统一显示“进行中”“已完成”或“失败”；点击任务可查看完整参数、阶段与结果。列表不扫描旧输出，服务重启后从空列表开始。
...
命令行可加 `--codex-service-tier fast` 使用 Fast 服务层；默认是 `default`。网页控制台默认勾选 Fast，可随时关闭。对一个已完成的视频报告，可加 `--regenerate` 先归档旧版再从 `analyze` 重新生成；没有该参数时会继续保持幂等复用。
...
目录还包含 `README.md` 和记录引擎、模型、推理强度及阶段结果的 `automation-run.json`。中间产物位于 `work/`，正式产物位于 `reports/`。显式重新生成的视频旧版位于 `work/_revisions/`、`reports/_revisions/` 和 `output/_revisions/`，版本索引记录在 `state/report-revisions.json`。
