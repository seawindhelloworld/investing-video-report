...
这个项目通过本机 Codex CLI 或 OpenCode，把输入内容生成可追溯的 Markdown 与 HTML 报告。现在有两种独立模式：

| 模式 | 输入 | 输出逻辑 |
| --- | --- | --- |
| 视频报告 | 一个完成结构与质量验证的标准字幕包 | 忠实整理视频作者内容，再做外部证据研判与 Agent 综合判断 |
...
视频报告只读取 `package_type = video_transcript` 的标准包。导入会验证 schema、SHA-256、字幕结构、上游审计记录和质量门，然后复制到 `work/VIDEO_ID/transcript/`。导入的 `transcript.corrected.jsonl` 永不覆盖。项目不再运行独立字幕勘误；报告模型在正常内容分析和写作中按语境理解明显的 ASR 错词，无法确认的词项保留不确定性。清洗遵循“无法判断就保留”和最小范围排除：广告、推广、订阅/销售话术、无关内容与片尾套话只可在片头/片尾窗口从筛选视图排除（每侧最多 120 秒，且不超过字幕总时长的三分之一），中段原文一律保留在字幕依据中；但在报告分析和成稿时，无论位置，识别为广告、推广、订阅或销售话术的内容都不会成为正文或报告观点。仅明显 ASR 噪声和空白段可在任何位置最小化排除。筛选产物保留来源哈希与逐项审计。项目不会下载视频、转码、执行 ASR 或清理上游目录。

视频 HTML 默认使用渐进披露：顶部提供三层阅读导航，每个主要主题先显示作者内容结论、外部证据增量和 Agent 决策增量，完整背景、限定、较长原话、详细判断字段与更多来源保留在同页可展开区域。草稿阶段会检查默认可见压缩率、长段跨层重复、三层 claim 映射与折叠审计内容；这些检查不会修改字幕，也不会增加独立模型调用。
